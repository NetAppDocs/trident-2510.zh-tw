...
有關驅動程式詳細資料、請參閱 link:../trident-use/ontap-nas.html["NAS 驅動程式"] 和 link:../trident-use/ontap-san.html["SAN 驅動程式"] 。
...
您可以使用  `ontap-nas` 驅動程式來配置 SMB 磁碟區。在完成 <<ONTAP SAN 和 NAS 驅動程式整合>> 之前，請先完成以下步驟：link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["準備配置 SMB Volume"]
