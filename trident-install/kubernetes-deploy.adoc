...
* 對執行受支援 Kubernetes 版本且已啟用相關功能需求的受支援 Kubernetes 叢集擁有完整權限。詳情請查看 link:../trident-get-started/requirements.html["要求"]。
* 可存取受支援的 NetApp 儲存系統。
...
選擇適合您的安裝方式。在做出決定之前，您還應該考慮 link:kubernetes-deploy.html#moving-between-installation-methods["方法之間的轉換"]。



=== 使用 Trident 運算子

無論手動部署或使用 Helm，Trident operator 都是簡化安裝和動態管理 Trident 資源的絕佳方式。您甚至可以link:../trident-install/kubernetes-customize-deploy.html["自訂您的 Trident 操作員部署"]使用 `TridentOrchestrator`自訂資源（CR）中的屬性。
