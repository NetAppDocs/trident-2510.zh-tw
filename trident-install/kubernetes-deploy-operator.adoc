...
summary: 您可以手動部署 Trident Operator 來安裝 Trident。此程序適用於 Trident 所需的容器映像未儲存在私人登錄中的安裝情況。 
---
...
[role="lead"]
您可以手動部署 Trident Operator 來安裝 Trident。此程序適用於 Trident 所需的容器映像未儲存在私人登錄中的安裝。如果您確實擁有私有映像登錄，請使用 link:kubernetes-deploy-operator-mirror.html["離線部署流程"]。
...
.開始之前
在開始安裝之前，請登入 Linux 主機並驗證它是否正在管理運作中的 link:../trident-get-started/requirements.html["支援的 Kubernetes 叢集"]，以及您是否擁有必要的權限。
...
現在您可以建立  `TridentOrchestrator` 並安裝 Trident。或者，您可以link:kubernetes-customize-deploy.html["自訂您的 Trident 安裝"]使用  `TridentOrchestrator` 規範中的屬性。
