...
是的，Trident 可以離線安裝。請參閱link:trident-install/kubernetes-deploy.html["了解 Trident 安裝"]。
...
Trident 安裝程式會建立一個 Kubernetes ClusterRole，該實例擁有對 Kubernetes 叢集的 PersistentVolume、PersistentVolumeClaim、StorageClass 和 Secret 資源的特定存取權。請參閱link:trident-install/kubernetes-customize-deploy-tridentctl.html["自訂 tridentctl 安裝"]。



=== 我可以在本機產生 Trident 用於安裝的確切資訊清單檔案嗎？

如有需要，您可以在本機產生和修改 Trident 安裝時使用的確切資訊清單檔案。請參閱link:trident-install/kubernetes-customize-deploy-tridentctl.html["自訂 tridentctl 安裝"]。
